...
data_header_charg_event=np.loadtxt('Data/D2_S4_C10_charging_event_sequence.txt',max_rows=1,dtype=int)
data_main_body_charg_event=np.loadtxt('Data/D2_S4_C10_charging_event_sequence.txt',skiprows=1,dtype=int)
...
data_header=np.loadtxt('Data/D2_S4_C10_a_trips.txt',max_rows=1,dtype=int)
data_header_decimal=np.loadtxt('Data/D2_S4_C10_a_trips.txt',max_rows=1,dtype=float)
data_main_body=np.loadtxt('Data/D2_S4_C10_a_trips.txt',skiprows=1,dtype=int)
...
for i in N:
    for j in N:
        ...
        t[i,j]=distance.euclidean([latitude_of_node_i_end,longitude_of_node_i_end],[latitude_of_node_j_start,longitude_of_node_j_start])
        bijk[i,j]=travel_cost*t[i,j]
        theta[i,j] = e_consumption * distance.euclidean([latitude_of_node_i_end, longitude_of_node_i_end],[latitude_of_node_j_start,longitude_of_node_j_start])
        if i==j:
            t[i,j]=0; theta[i,j]=0
...
model.addConstrs(T[i,k]+t_tilde[i]+t[i,j]+sigma[i,j,k]<=u[j] for k in K for i,j in A[k] if i in Vk[k]) #eq.(54)
model.addConstrs(T[i,k]+tau[i,k]+t[i,j]+sigma[i,j,k]<=u[j] for k in K for i,j in A[k] if i in F) #eq.(55)
for k in K:
    for k2 in K:
        if k2!=k:
            for i,j in A[k]:
                for i2,j2 in A[k2]:
                    if i==i2 and j==j2:
                        model.addConstr(x[i,j,k]+x[i2,j2,k2]<=1) #eq.(56)
for k in K:
    for i,j in A[k]:
        for i2,j2 in A[k]:
            if i==i2 and j!=j2:
                model.addConstr(x[i, j, k] + x[i2, j2, k] <= 1) #eq.(57)
            if j==j2 and i!=i2:
                model.addConstr(x[i, j, k] + x[i2, j2, k] <= 1) #eq.(58)
#model.addConstr(x[i,j,k]+x[i2,j2,k2]<=1 for k in K for k2 in K if k2!=k for i,j in A[k] for i2,j2 in A[k2] if i==i2 and j==j2)
#model.addConstrs(x[i,j,k]+x[i2,j2,k]<=1 for k in K for i,j in A[k] for i2,j2 in A[k] if i2==i)
#model.addConstrs(x[i,j,k]+x[i2,j2,k]<=1 for k in K for i,j in A[k] for i2,j2 in A[k] if j2==j)
